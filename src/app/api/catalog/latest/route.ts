...
export async function GET(request: NextRequest) {
  ...
  const catalog = await db
    ...
    .fullJoin(
      Promotion,
      and(
        eq(Promotion.catalog_id, Catalog.id),
    ...
    .where(
      and(
        eq(
          Photo.order,
          db
            .select({ order: sql`MIN(${Photo.order})` })
            .from(Photo)
            .where(eq(Photo.variant_id, Variant.id))
        ),
        ...(showOnlyPromos ? [eq(Promotion.active, true)] : [])
      )
    );
  ...
  catalog.forEach((item) => {
    if (!item?.catalog || !item?.product || !item?.category) return;

    if (!showWithoutStock && !item?.catalog?.show) return;

    const categoryID = item?.category?.id || 'noCategory';
    const CategoryName = item?.category?.name ? t(item.category.name) : t_catalog('noCategory');
    const CategoryDescription = item?.category?.description ? t(item.category.description) : t_catalog('noDescription');
    ...
    const registered = !!item?.product?.id;
    const originalName = item?.product?.name ? t(item.product.name) : t_catalog('noName');
    const review = item?.product?.review ? t(item.product.review) : t_catalog('noReview');
    const included = item?.product?.included ? t(item.product.included) : null;
    const specs = item?.product?.specs ? t(item.product.specs) : t_catalog('noSpecs');
    const photo = item?.photo?.url || 'noPhoto';
    const color = t(item?.color?.color || '') || 'noColor';

    // ✅ Promoción activa o nula
    const promotion =
      item?.promotion
        ? {
            ...
            data: {
              ...item?.promotion,
              data:
                typeof item?.promotion?.data === 'string'
                  ? JSON.parse(item.promotion.data)
                  : item.promotion.data
            }
          }
        : null;
    ...
    const product: ProductData = {
      code: `${code}${
        (item?.promotion && item.promotion.type_id) ? `-${item.promotion.label}-${item.promotion.id}` : ''
      }`,
  ...
  const orderedProducts: GroupedByCategory = {
    ...categoryOrder.reduce((acc: GroupedByCategory, category: string) => {
      if (groupedByCategory[category]) acc[category] = groupedByCategory[category];
      return acc;
    }, {}),
